  ...
  namespace explore {
    ...
    namespace details {
               ...
               tchecker::log_t & log, bool eqOrSim)
      {
        ...
        using graph_t = tchecker::explore::graph_t<graph_allocator_t, hash_t, equal_to_t, graph_outputter_t, ts_t>;
        // std::cout << "EXTRA1\n";
        model_t model(sysdecl, log);
        // std::cout << typeid(edge_outputter_t).name() << "\n";
        // std::cout << "EXTRA2\n";
        ...
        //   std::cout << (**iter).pid() << " " << (**iter).id() << " " << (**iter).name() << "\n";
        // }
        // auto k = sysdecl._clocks;
        std::unordered_map<std::string, tchecker::parsing::clock_declaration_t const *> y = sysdecl._clocks;
        std::unordered_map<std::string, int> clocks;
        // auto events = sysdecl._events;
        
        // auto sys = model.system();
        // auto events = sys.events();
        // std::string event = events.value(0);
