...
namespace tchecker {
  ...
  namespace explore {
     ...
     */
    template <class ALLOCATOR, class HASH, class EQUAL_TO, class OUTPUTTER, class TS>
    class graph_t
    : public ALLOCATOR,
    public tchecker::graph::find_graph_t<typename ALLOCATOR::node_ptr_t, HASH, EQUAL_TO, TS>,
    ...
    public:
       ...
       */
      graph_t(tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> const &) = delete;
      
      /*!
       \brief Move constructor (deleted)
       */
      graph_t(tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> &&) = delete;
       ...
       */
      tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> &
      operator= (tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> const &) = delete;
      
      /*!
       \brief Move-assignment operator (deleted)
       */
      tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> &
      operator= (tchecker::explore::graph_t<ALLOCATOR, HASH, EQUAL_TO, OUTPUTTER, TS> &&) = delete;
      ...
      void destruct_all()
      {
        tchecker::graph::find_graph_t<typename ALLOCATOR::node_ptr_t, HASH, EQUAL_TO, TS>::clear();
      ...
      {
        if (! tchecker::graph::find_graph_t<typename ALLOCATOR::node_ptr_t, HASH, EQUAL_TO, TS>::add_node(node_ptr))
          return false;
...
=======
      ...
      bool add_node(node_ptr_t & node_ptr, std::string &state){
        if(!tchecker::graph::find_graph_t<typename ALLOCATOR::node_ptr_t, HASH, EQUAL_TO, TS>::add_node(node_ptr, state))
          return false;
        return tchecker::graph::tracing_graph_t<node_ptr_t, edge_ptr_t, OUTPUTTER>::add_node(node_ptr);
      }

      void out_node(std::pair<node_ptr_t, node_ptr_t> const &node){
        tchecker::graph::tracing_graph_t<node_ptr_t, edge_ptr_t, OUTPUTTER>::out_node(node);
      }

      void output_node(node_ptr_t const &node){
        tchecker::graph::tracing_graph_t<node_ptr_t, edge_ptr_t, OUTPUTTER>::output_node(node);
      }

      void output_edge(edge_ptr_t & edge_ptr){
        tchecker::graph::tracing_graph_t<node_ptr_t, edge_ptr_t, OUTPUTTER>::output_edge(edge_ptr);
      }

      std::pair<std::vector<std::tuple<std::string, int, int>>, std::vector<std::string>> get_g_n_r(edge_ptr_t & edge_ptr){
        return tchecker::graph::tracing_graph_t<node_ptr_t, edge_ptr_t, OUTPUTTER>::get_g_n_r(edge_ptr);
      }
