...
namespace tchecker {
  ...
  template <class CLKCONSTR_ITERATOR>
  std::vector<std::tuple<std::string, int, int>> get_g(tchecker::range_t<CLKCONSTR_ITERATOR> const & range,
                                                                                                tchecker::clock_index_t const &index){
    std::vector<std::tuple<std::string, int, int>> gs;
    auto begin = range.begin(), end = range.end();
    for (auto it = begin ; it != end ; it++){
      gs.push_back(tchecker::get_g(*it, index));
    }
  ...
  template <class CLKRESET_ITERATOR>
  std::vector<std::string> get_r(tchecker::range_t<CLKRESET_ITERATOR> const & range,
                                  tchecker::clock_index_t const & index){
    std::vector<std::string> rs;
    auto begin = range.begin(), end = range.end();
    for(auto it = begin ; it != end ; it++)
      rs.push_back(get_r(*it, index));
    return rs;
  }
